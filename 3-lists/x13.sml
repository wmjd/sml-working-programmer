(*Alter allChange to use a fixed purse of coins*)

val romanPurse = [1000,500,100,50,10,5,1];
...
|   allChange (cs, c::cv, amt) =
	if amt<0 then []
		else allChange(c::cs, cv, amt-c) @ allChange(cs, cv, amt) ;

fun decToRoman n = converts(allChange([], romanPurse, n));
